...
do
{
    sudo ./simple_switch motivation.json $INTF_1 $INTF_S1_2 $LOG --thrift-port 9090
} &
done
...
do
{
    sudo ./simple_switch motivation1.json $INTF_S2_1 $INTF_S2_2 $LOG --thrift-port 9091
} &
done

sudo ./simple_switch motivation1.json $INTF_S3_1 $INTF_2 $LOG --thrift-port 9092